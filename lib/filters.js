...
/** Slugifies each "/"-separated segment of a category path independently, e.g. "AI and ML/Rust"
 * becomes "ai-and-ml/rust" instead of one slug for the whole string. */
export const slugifyPath = (path = '') => {
  return String(path)
    .split('/')
    .map((segment) => slugifyString(segment))
    .join('/');
};

/** True if a post's category (e.g. ["Programming", "Rust"]) falls under or exactly matches the
 * given "/"-separated category path (e.g. "Programming"), so a parent category page also picks up
 * posts from its subcategories. */
export const inCategory = (postCategory = [], categoryPath = '') => {
  const target = String(categoryPath).split('/');
  return target.every((segment, i) => postCategory[i] === segment);
};
