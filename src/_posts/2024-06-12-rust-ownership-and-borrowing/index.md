...
description: >-
  Rust's ownership and borrowing features prevent us from experiencing
  memory-related problems. Rust is a great choice when performance matters and
  it solves pain points that bother many other languages.
keywords:
  - ownership
  - borrowing
  - memory-management
tags:
  ...
  - borrowing
  - memory-management
coverImage: ./images/rust-ownership-borrowing.png
imageDescription: >-
  The harmonious interaction between dog owners and their pets serves as a
  metaphor for the Rust programming language's approach to memory management
...
{% include "postImage.html" src: "./images/protests-by- garbage-collectors.webp", alt: "Generated by DALL.E", description: "<b>Figure 1: </b>Depicting the scene of garbage collectors protesting over job loss, serving as a metaphor for how Rust's ownership and borrowing system eliminates the need for a garbage collector.", credits: "Generated by DALL-E." %}
...
{% include "postImage.html" src: "./images/stealing.jpg", alt: "Designed by Freepik", description: "<b>Figure 2: </b>Stealing.", credits: "Designed by Freepik" %}
...
{% include "postImage.html" src: "./images/leakage.jpg", alt: "Designed by Freepik", description: "<b>Figure 3: </b>Leakage.", credits: "Designed by Freepik" %}
...
{% include "postImage.html" src: "./images/memory-unsafety-vs-memory-leaks.png", alt: "Created by Author", description: "<b>Figure 4: </b>Memory unsafety vs. memory leaks." %}
...
{% aside %} All primitive types in Rust live on the stack. Types like numbers, characters, slices, booleans, fixed-size arrays, tuples containing primitives, and function pointers can all sit on the stack. {% endaside %}
...
{% aside %} Garbage collection is too costly in systems languages like C and C++, therefore explicit memory allocation continues to exist. {% endaside %}
...
{% aside %} It’s entirely our (developers) responsibility to ensure that no pointer to the value we freed is ever used. {% endaside %}
...
{% include "postImage.html" src: "./images/rust-stands-out.png", alt: "Created by Author", description: "<b>Figure 5: </b>Rust has better control over memory management and provide higher safety with no memory issues." %}
...
{% include "postImage.html" src: "./images/explicit-mem-vs-implicit-mem.png", alt: "Created by Author", description: "<b>Figure 6: </b>Explicit memory management vs. Implicit memory management vs. Rust’s ownership model." %}
...
{% include "postImage.html" src: "./images/rust-ownership.webp", alt: "Created by Author", description: "<b>Figure 7: </b>Metaphorically reflects Rust's ownership.", credits: "Image generated by DALL-E." %}
...
{% include "postImage.html" src: "./images/variable-binding.png", alt: "Created by Author", description: "<b>Figure 8: </b>Variable binding shows the owner and its value/resource." %}
...
{% aside %} It's worth noting that primitive types with fixed-size implement the copy trait to make copies. {% endaside %}
...
{% aside %} In Rust, there are two kinds of strings: `String` (heap allocated, and growable) and `&str` (fixed size, and can’t be mutated). {% endaside %}
...
{% include "postImage.html" src: "./images/stack-with-own-data.png", alt: "Created by Author", description: "<b>Figure 8: </b>Both `x` and `y` have their own data." %}
...
{% include "postImage.html" src: "./images/stack-holds-metadata.png", alt: "Created by Author", description: "<b>Figure 10: </b>The stack holds the metadata while the heap holds the actual contents." %}
...
{% include "postImage.html" src: "./images/variable-s2-gets-a-copy.png", alt: "Created by Author", description: "<b>Figure 11: </b>Variable `s2` gets a copy of the pointer, length, and capacity of `s1`." %}
...
{% include "postImage.html" src: "./images/data-replication.png", alt: "Created by Author", description: "<b>Figure 12: </b>If Rust copied the heap data, another possibility for what `let s2 = s1` might do is data replication. However, Rust does not copy by default." %}

Note that when complex types are no longer in scope, Rust will call the `drop` function to explicitly deallocate heap memory. However, both data pointers in **Figure 11** are pointing to the same location, which is not how Rust works. We will get into the details shortly.
...
{% include "postImage.html" src: "./images/after-s1-invalidated.png", alt: "Created by Author", description: "<b>Figure 13: </b>Memory representation after `s1` has been invalidated." %}
...
{% include "postImage.html" src: "./images/heap-data-copied-into-s2.png", alt: "Created by Author", description: "<b>Figure 14: </b>When using the clone method, the heap data does get copied into `s2`." %}
...
{% include "postImage.html" src: "./images/a-guy-walking-with-dogs.jpeg", alt: "Designed by Freepik", description: "<b>Figure 15: </b>A guy walking with dogs." %}
...
{% include "postImage.html" src: "./images/compiler-copies-x-to-y-and-z.png", alt: "Created by Author", description: "<b>Figure 16: </b>Compiler made copies of `x` to both `y` and `z`." %}
...
{% aside %} In the case of `String` type, the size might expand and be stored on the heap. This means:
...
Rust handles memory deallocation in a novel way to make our lives easier: the memory is automatically returned once the variable that owns it goes out of scope. {% endaside %}
...
{% aside %} When complex types are no longer in scope, Rust will call the `drop` function to explicitly deallocate heap memory. {% endaside %}
...
{% include "postImage.html" src: "./images/mem-representation-after-s1-dropped.png", alt: "Created by Author", description: "<b>Figure 17: </b>Memory representation after `s1` has been dropped." %}
...
{% include "postImage.html" src: "./images/nervous-breakdown.png", alt: "Designed by Freepik", description: "<b>Figure 18: </b>Woman going through nervous breakdown." %}
...
{% include "postImage.html" src: "./images/a-girl-borrows-a-book-from-a-librarian.webp", alt: "Designed by Freepik", description: "<b>Figure 19: </b>A girl borrows a book from a librarian. Generated by DALL-E." %}
...
{% aside %} **Dereference:** In the above code, "*" (i.e., an asterisk) is a *dereference* operator that operates on a reference variable. This dereferencing operator allows us to get the value stored in the memory address of a pointer. {% endaside %}
...
{% include "postImage.html" src: "./images/stack-mem-representation.png", alt: "Created by Author", description: "<b>Figure 20: </b>Stack memory representation." %}
...
{% aside %} Immutable references are also known as *shared references*, whereas mutable references are also known as *exclusive references*. {% endaside %}
